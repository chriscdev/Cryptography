...
namespace Cryptography.Library.Test.Benchmarks
{
  ...
  {
    private readonly Rsa _rsa1024 = new(RsaKeyFactory.Create1024BitPrivateKey());
    private readonly Rsa _rsa2048 = new(RsaKeyFactory.Create2048BitPrivateKey());
    private readonly Aes _aes = new(Convert.FromBase64String("LH69rYyuaO5mWYj+daDXRY46x9ta0JFhTUWXT5HYumA="), Convert.FromBase64String("wlF5jKFqVcH6IaXiVahYOQ=="));
    private readonly FastRsa _fastRsa1024 = new(RsaKeyFactory.Create1024BitPrivateKey(), "gfRvtMnf9ZklJJ1u6opDwqN2Psh1NULbZAZ7DsBVAlns6T7rEs1OFG0wvHwUfjbZoH4Nnz6LDjJoXwoi2+7COaRpFnWHdQf0S0jA4ZOdYibgi9opPB8y2sXENOKDyVhsAiVZOp9PiaeKRiiLiXcFaJtrw4gJvecEObbUwbfSe3Q=");
    private readonly FastRsa _fastRsa2048 = new(RsaKeyFactory.Create2048BitPrivateKey(), "");
    private readonly FastRsa _fastRsa1024CacheEnabled = new(RsaKeyFactory.Create1024BitPrivateKey(), true);
    private readonly FastRsa _fastRsa2048CacheEnabled = new(RsaKeyFactory.Create2048BitPrivateKey(), true);
    ...
    [Fact(Skip = "Do not run in pipeline")]
    ...
    [Benchmark]
    public string FastRsaDecrypt_1024Bit_CacheEnabled() => _fastRsa1024CacheEnabled.Decrypt(_fastRsa1024BitShortCipher);

    [Benchmark]
    public string FastRsaDecrypt_2048Bit_CacheEnabled() => _fastRsa2048CacheEnabled.Decrypt(_fastRsa2048BitShortCipher);
  }
}
